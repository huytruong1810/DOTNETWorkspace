...
public class Board
{
    ...
    public string MoveToLongAlgebraic(int x, int y, int destX, int destY, bool isCapture)
    {
        ...
        return $"{pieceChar}{(char)('a' + y)}{8 - x}{(isCapture ? "x" : "-")}{(char)('a' + destY)}{8 - destX}";
    }
...
public abstract class Piece(bool isWhite)
{
    ...
    {
        if (!InBorders(x, y) || !board.IsOccupied(x, y)) return; // For pawn case
        ...
        int maxSteps)
    {
        ...
        {
            if (maxSteps == 0) break; // Meet allowed number of incrementation
